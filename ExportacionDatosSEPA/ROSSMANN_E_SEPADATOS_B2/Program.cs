...
namespace E_PAYROLL_SEPA_B2
{
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            if (!Path.Exists(program.appSettings.DirectorioFicherosGenerados))
            {
                Directory.CreateDirectory(program.appSettings.DirectorioFicherosGenerados);
            }
        ...
        bool MainIntegration_Contabilidad(string CaptioCustomerKey_v3, ref List<string> reportsProcessats)
        {
            ...
            exportDATA.AddRange(ProcessReports(fechaInicioExport_txt, fechaFinExport_txt, ref usersCaptio, ref reportsProcessats, categoriesCaptio));
            ...
            if (exportDATA.Count > 0)
            {
                try
                {
                    ...
                    File.WriteAllText(Path.Combine(appSettings.DirectorioFicherosGenerados, nombreFichero), bbb, codificacion);
                    utils_fitxers.ConvertCSV_to_XLSX(Path.GetFileNameWithoutExtension(nombreFichero), Path.GetExtension(nombreFichero), appSettings.DirectorioFicherosGenerados, true);
                    

                    // Log
                    Log.Info("GENERADO FICHERO DE Contabilidad [" + Path.Combine(appSettings.DirectorioFicherosGenerados, nombreFichero) + "]");
        ...
        private List<FicheroExportacionDTO> ProcessReports(string FechaInicio, string FechaFin, ref List<UsersDTO_v3_1> usersCaptio, ref List<string> reportsProcessatsTotals, List<CompaniesCategoriesDTO_v3_1> categoriasCaptio)
        {
            try
            {
                ...
                if ((reports != null) && (reports.Length > 0))
                {
                    ...
                    foreach (string id_user in reports.Select(x => x.User.Id).Distinct())
                    {
                        ...
                        List<ReportsDTO_v3_1> reports_x_usuari = new List<ReportsDTO_v3_1>();
                        reports_x_usuari.AddRange(Array.FindAll(reports, r => r.User.Id.Equals(id_user)).ToList());

                        // Log
                        Log.Info("PROCESANDO REPORTS DEL USUARIO : [" + user.Login.ToString() + "] - [" + user.Name + "]");

                        // Generar las líneas dels fitxer de cada usuario
                        bool usuario_ok = FicheroExportacion_Gastos(ref exportData, user, reports_x_usuari, categoriasCaptio);
                        if (usuario_ok)
                        {
                            foreach (ReportsDTO_v3_1 report in reports_x_usuari)
                            {
                                // Log
                                Log.Info(" --> Report " + report.ExternalId.Trim() + " del usuario [" + user.Login + "] añadido al fichero de exportación");

                                // Afegir id a reports processats
                                reportsProcessatsTotals.Add(report.Id);
                            }
                        }
                        else
                        {
        ...
        private FicheroExportacionDTO AddFirstLine()
        {
            FicheroExportacionDTO result = (new FicheroExportacionDTO()
            {
                ...
                Campo_4 = "Amount",
                Campo_5 = "Fecha generación",
                Campo_6 = "Código SUMMAR",
        ...
        private bool FicheroExportacion_Gastos(ref List<FicheroExportacionDTO> exportData, UsersDTO_v3_1 user, List<ReportsDTO_v3_1> reports, List<CompaniesCategoriesDTO_v3_1> categoriasCaptio)
        {
            ...
            if (!report_amb_Violations)
            {
                ...
                {
                    //suma_total = suma_total + decimal.Parse(report.ReimbursableAmount, CultureInfo.GetCultureInfo("en-US"));
                    decimal total_reimbursableAmount_Rossman = ObtenerReembolsable_Report_Rossmann(report, categoriasCaptio);
                    suma_total = suma_total + total_reimbursableAmount_Rossman;
                }
                ...
                try
                {
                    result.Add(new FicheroExportacionDTO()
                    {
                        ...
                        Campo_4 = suma_total.ToString("0.00", CultureInfo.GetCultureInfo("es-ES")),
                        //Campo_5 = fechaGeneracion.ToString("dd/MM/yyyy")
                        Campo_5 = (Obtener_Fecha_Generacion_SUMMAR(fechaGeneracion)).ToString("dd/MM/yyyy"),
                        Campo_6 = "K888"
                    });
                    num_linea++;
        ...
        private decimal ObtenerReembolsable_Report_Rossmann(ReportsDTO_v3_1 report, List<CompaniesCategoriesDTO_v3_1> categoriasCaptio)
        {
            decimal result = 0;

            List<ExpensesDTO_v3_1> expenses = new List<ExpensesDTO_v3_1>();

            expenses.AddRange(captio_v3.GetExpenses_v3_1("\"" + "Report_id" + "\"" + ":" + report.Id));

            foreach (ExpensesDTO_v3_1 expense in expenses)
            {
                if ((expense.PaymentMethod != null) && (!String.IsNullOrEmpty(expense.PaymentMethod.IsReimbursable)) && (expense.PaymentMethod.IsReimbursable.ToLower().Equals("true")))
                {
                    // Excluir los tickets DIETA que tengan un valor igual o superior al límite
                    if (!EsTicketDieta_Superior_O_Igual_al_Limite_Rossmann(expense, categoriasCaptio))
                    {
                        result = result + decimal.Parse(expense.FinalAmount.Value, CultureInfo.GetCultureInfo("en-US"));
                    }
                }
